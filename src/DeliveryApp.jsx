...
/* ============================================================
   ...
   خلفية لوحة عداد غامقة + بطاقات شركات بألوان مميزة + أرقام كبيرة واضحة
   ============================================================ */
const COLORS = {
  bg: "#11151c",
  bgCard: "#1b212b",
...
const DEFAULT_DATA = {
  exchangeRate: 89000, // ليرة لكل دولار
  users: [], // {id, username, password, displayName}
  companies: [], // {id, name, color, icon}
  entries: [], // كل العمليات: طلب/دين/تسديد/مصروف
  closures: [], // تسكير حساب لكل شركة بتاريخ
};
      ...
      <div style={{ background: COLORS.bgCard, borderRadius: 18, padding: 20, border: `1px solid ${COLORS.border}` }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 18, background: COLORS.bgCard2, borderRadius: 12, padding: 4 }}>
          <button
            ...
            style={{
              flex: 1, padding: "10px", borderRadius: 10, border: "none", fontWeight: 700, cursor: "pointer",
              background: mode === "login" ? COLORS.green : "transparent",
              color: mode === "login" ? "#fff" : COLORS.textDim,
            }}
          >
            تسجيل دخول
          </button>
          <button
            ...
            style={{
              flex: 1, padding: "10px", borderRadius: 10, border: "none", fontWeight: 700, cursor: "pointer",
              background: mode === "register" ? COLORS.green : "transparent",
              color: mode === "register" ? "#fff" : COLORS.textDim,
            }}
          >
            حساب جديد
          </button>
        ...
        <Field label="كلمة المرور">
          <div style={{ position: "relative" }}>
            ...
            <button
              ...
              style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: COLORS.textDim, cursor: "pointer" }}
            >
...
export default function DeliveryApp() {
  ...
  const persist = useCallback(async (updater) => {
    setData((prev) => {
      ...
      saveData(next)
        .catch(() => {
          // فشل الحفظ (غالباً لانقطاع الإنترنت) — Firestore بيعيد المحاولة تلقائياً لما يرجع الاتصال
          skipNextSnapshot.current = false;
        })
  ...
  if (screen === "home") {
    content = (
      <HomeScreen
        data={data}
        persist={persist}
        onOpenCompany={(id) => { setSelectedCompanyId(id); setScreen("company"); }}
        onAddCompany={() => setScreen("addCompany")}
        onOpenConvert={() => setScreen("convert")}
        onOpenSettings={() => setScreen("settings")}
        onQuickAction={quickStart}
      />
    );
    ...
    content = (
      <AddCompanyScreen
        ...
        onSave={(name, color) => {
          persist((prev) => ({
            ...prev,
            companies: [...prev.companies, { id: uid(), name, color }],
          }));
    ...
    content = (
      <CompanyScreen
        ...
        currentUser={currentUser}
        onBack={() => setScreen("home")}
        onAction={(type) => { setEntryFormType(type); setEditingEntryId(null); setScreen("entryForm"); }}
        onEditEntry={(entry) => { setEntryFormType(entry.type); setEditingEntryId(entry.id); setScreen("entryForm"); }}
        onCloseAccount={() => setScreen("closeAccount")}
        showToast={showToast}
        onDeleteCompany={() => {
          persist((prev) => ({
            ...prev,
            companies: prev.companies.filter((c) => c.id !== company.id),
            entries: prev.entries.filter((e) => e.companyId !== company.id),
          }));
          setScreen("home");
        }}
      />
    ...
    content = (
      <EntryFormScreen
        ...
        onBack={() => { setEditingEntryId(null); setScreen("company"); }}
        onSave={(entry) => {
          persist((prev) => {
            if (editingEntryId) {
              return { ...prev, entries: prev.entries.map((e) => (e.id === editingEntryId ? entry : e)) };
            }
            return { ...prev, entries: [...prev.entries, entry] };
          });
          showToast(editingEntryId ? "تم تعديل العملية" : "تم الحفظ");
          setEditingEntryId(null);
          setScreen("company");
        }}
      />
      ...
      >
        ...
        {!pickedCompany ? (
          <>
            ...
            {companies.length === 0 ? (
              <div style={{ textAlign: "center", color: COLORS.textFaint, padding: "20px 0", fontSize: 14 }}>
                لا توجد شركات. أضف شركة من الرئيسية أولاً.
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {companies.map((c) => (
                  <button
                    ...
                    style={{
                      display: "flex", alignItems: "center", gap: 12, background: COLORS.bgCard2,
                      border: `1px solid ${COLORS.border}`, borderRadius: 14, padding: 12, cursor: "pointer", textAlign: "right",
                    }}
                  >
                    <div style={{ width: 38, height: 38, borderRadius: 10, background: c.color, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#fff" }}>
                      {c.name.slice(0, 1)}
                    </div>
          ...
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              <button onClick={() => setPickedCompany(null)} style={{ background: "none", border: "none", color: COLORS.textDim, cursor: "pointer", padding: 4 }}>
                <ChevronRightIcon size={20} />
              </button>
              <div style={{ width: 32, height: 32, borderRadius: 8, background: pickedCompany.color, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#fff", fontSize: 13 }}>
                {pickedCompany.name.slice(0, 1)}
              </div>
              <div style={{ fontWeight: 800, fontSize: 15 }}>{pickedCompany.name}</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {actions.map((a) => (
                <BigButton key={a.type} icon={a.icon} label={a.label} color={a.color} onClick={() => onPick(pickedCompany.id, a.type)} />
              ))}
            </div>
          </>
        )}
    ...
    <div style={{ padding: "16px 16px 0" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        ...
        <button
          ...
          style={{ background: "none", border: "none", color: COLORS.green, fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}
        >
      ...
      {data.companies.length === 0 && (
        <div style={{ textAlign: "center", color: COLORS.textFaint, padding: "40px 0", fontSize: 14 }}>
          لا توجد شركات بعد. اضغط "إضافة شركة" للبدء
        </div>
      )}
          ...
          return (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: COLORS.textDim }}>
                    الطلبات اليوم: {todayCount} &nbsp;|&nbsp; الأرباح: <span style={{ color: COLORS.green }}>${fmt(s.profit, 2)}</span>
                  </div>
                  <div style={{ fontSize: 12, color: COLORS.textDim, marginTop: 2 }}>
                    الدين: <span style={{ color: s.debt > 0 ? COLORS.orange : COLORS.textFaint }}>${fmt(s.debt, 2)}</span>
                  </div>
                </div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  onClick={(ev) => { ev.stopPropagation(); onQuickAction(c.id, "order"); }}
                  style={{
                    flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                    background: COLORS.green, border: "none", borderRadius: 10, padding: "9px 0",
                ...
                <button
                  onClick={(ev) => { ev.stopPropagation(); onQuickAction(c.id, "expense"); }}
                  style={{
                    flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                    background: COLORS.red, border: "none", borderRadius: 10, padding: "9px 0",
                    color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer",
                  }}
                >
                  <Plus size={15} /> مصروف
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {COMPANY_COLORS.map((c) => (
              <button
                ...
                onClick={() => setColor(c)}
                style={{
                  width: 40, height: 40, borderRadius: 12, background: c, border: color === c ? "3px solid #fff" : "3px solid transparent",
                  cursor: "pointer",
                }}
              />
            ))}
  ...
  return (
    <div>
      <TopBar
        ...
        right={
          <div style={{ position: "relative" }}>
            <button onClick={() => setMenuOpen((m) => !m)} style={{ background: "none", border: "none", color: COLORS.text, cursor: "pointer" }}>
              ⋮
            </button>
            {menuOpen && (
              <div style={{ position: "absolute", top: 30, left: 0, background: COLORS.bgCard2, border: `1px solid ${COLORS.border}`, borderRadius: 10, overflow: "hidden", zIndex: 20, minWidth: 140 }}>
                <button
                  onClick={() => { setMenuOpen(false); onDeleteCompany(); }}
                  style={{ display: "block", width: "100%", padding: "10px 14px", background: "none", border: "none", color: COLORS.red, fontSize: 13, fontWeight: 700, textAlign: "right", cursor: "pointer" }}
                >
                  حذف الشركة
                </button>
              </div>
      ...
      <div style={{ padding: "0 16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          <div style={{ width: 56, height: 56, borderRadius: 14, background: company.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 800, color: "#fff" }}>
        ...
        <button
          ...
          style={{
            width: "100%", background: COLORS.bgCard2, border: `1px solid ${COLORS.border}`, borderRadius: 14,
            padding: "14px", color: COLORS.text, display: "flex", alignItems: "center", justifyContent: "center",
            gap: 8, fontWeight: 700, cursor: "pointer", marginBottom: 20, fontSize: 15,
          }}
        >
        ...
        <div style={{ position: "relative", marginBottom: 12 }}>
          <input
            ...
            placeholder="بحث بقيمة الطلب أو رقمه..."
            inputMode="decimal"
          />
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              style={{ position: "absolute", insetInlineEnd: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: COLORS.textFaint, cursor: "pointer" }}
            >
              <X size={16} />
            </button>
          )}
        ...
        {entries.length === 0 && (
          <div style={{ textAlign: "center", color: COLORS.textFaint, padding: "30px 0", fontSize: 14 }}>
            {search ? "لا توجد نتائج مطابقة" : "لا توجد عمليات بعد"}
          </div>
        )}
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
          {entries.map((e) => (
            <EntryRow key={e.id} entry={e} onDelete={() => deleteEntry(e.id)} onEdit={() => onEditEntry(e)} />
          ))}
        </div>
      </div>
    </div>
...
function EntryRow({ entry, onDelete, onEdit }) {
  ...
  let mainAmount, sign;
  if (entry.type === "order") {
    mainAmount = entry.dueToCompanyUSD;
    sign = "$";
  } else {
    mainAmount = entry.amountUSD;
    sign = "$";
  }
        ...
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${COLORS.border}`, fontSize: 13, color: COLORS.textDim, display: "flex", flexDirection: "column", gap: 4 }}>
          {entry.type === "order" && (
            <>
              ...
              <Row
                label="التيبس"
                value={`${entry.tipsTotalUSD >= 0 ? "+" : ""}${fmtDual(entry.tipsUSD, entry.tipsLBP)}`}
                valueColor={entry.tipsTotalUSD >= 0 ? COLORS.green : COLORS.red}
              />
          ...
          <div style={{ display: "flex", gap: 14, marginTop: 6 }}>
            <button
              onClick={onEdit}
              style={{ background: "none", border: "none", color: COLORS.blue, fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}
            >
              <Edit3 size={13} /> تعديل
            </button>
            <button
              onClick={onDelete}
              style={{ background: "none", border: "none", color: COLORS.red, fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}
            >
              <Trash2 size={13} /> حذف
            </button>
          </div>
    ...
    <Field label={label}>
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{ position: "relative", flex: 1 }}>
          <input
            style={{ ...inputStyle, paddingInlineStart: 28 }}
            type="number"
            inputMode="decimal"
            value={usd}
        ...
        <div style={{ flex: 1 }}>
          <div style={{ position: "relative" }}>
            <input
              ...
              inputMode="decimal"
              value={lbp}
              onChange={(e) => onLbpChange(e.target.value)}
              placeholder="0"
            />
          ...
          {lbp !== "" && (
            <div style={{ fontSize: 11, color: COLORS.textFaint, marginTop: 4, textAlign: "left" }}>
              = {fmtLBP(lbpActual)} ل.ل
            </div>
          )}
        </div>
      </div>
...
function EntryFormScreen({ type, company, companies, data, currentUser, editingEntry, onBack, onSave }) {
  if (type === "order") {
    return <OrderForm company={company} companies={companies} data={data} currentUser={currentUser} editingEntry={editingEntry} onBack={onBack} onSave={onSave} />;
  }
...
function OrderForm({ company, companies, data, currentUser, editingEntry, onBack, onSave }) {
  ...
  const save = () => {
    onSave({
      ...
      paidLBP: pLBP,
      paidTotalUSD,
      tipsUSD: tipsTotalUSD,
      tipsLBP: tipsTotalLBP,
      tipsTotalUSD,
      orderNumber: orderNumber.trim(),
      note: note.trim(),
      createdAt: new Date(dateTime).getTime() || Date.now(),
      byName: e ? e.byName : currentUser.displayName,
    });
  };
    ...
    <div>
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        <div style={{ background: COLORS.bgCard2, borderRadius: 12, padding: 12, marginBottom: 14 }}>
          <Row
            label="التيبس"
            value={`${tipsTotalUSD >= 0 ? "+" : ""}$${fmt(tipsTotalUSD)}  (${fmtLBP(tipsTotalLBP)} ل.ل)`}
            valueColor={tipsTotalUSD >= 0 ? COLORS.green : COLORS.red}
          />
          {tipsTotalUSD < 0 && (
            <div style={{ fontSize: 12, color: COLORS.textFaint, marginTop: 4 }}>
              المقبوض أقل من المطلوب — هذا فقط للعرض، لا يُسجَّل كدين
            </div>
          )}
        </div>

        <Field label="رقم الطلب (اختياري)">
          <input style={inputStyle} value={orderNumber} onChange={(ev) => setOrderNumber(ev.target.value)} placeholder="#1258" />
        </Field>
        <Field label="ملاحظات (اختياري)">
          <input style={inputStyle} value={note} onChange={(ev) => setNote(ev.target.value)} placeholder="مثال: طلب من مطعم..." />
        </Field>
        <Field label="التاريخ والوقت">
          <input style={inputStyle} type="datetime-local" value={dateTime} onChange={(ev) => setDateTime(ev.target.value)} />
        </Field>
...
function SimpleForm({ type, company, companies, data, currentUser, editingEntry, onBack, onSave }) {
  ...
  const save = () => {
    onSave({
      ...
      amountUSD: isDual ? amtTotalUSD : amtUSD,
      amountLBP: isDual ? amtLBPReal : 0,
      rawUSD: amtUSD,
      reason: reason.trim(),
      note: note.trim(),
      createdAt: new Date(dateTime).getTime() || Date.now(),
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        {hasDirection && (
          <Field label="نوع الدين">
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={() => setDirection("owedToMe")}
                style={{
                  flex: 1, padding: "12px", borderRadius: 10, border: "none", fontWeight: 700, cursor: "pointer",
                  background: direction === "owedToMe" ? COLORS.green : COLORS.bgCard2,
                  color: direction === "owedToMe" ? "#fff" : COLORS.textDim,
                }}
              >
                دين لي (على الشركة)
              </button>
              <button
                onClick={() => setDirection("owedByMe")}
                style={{
                  flex: 1, padding: "12px", borderRadius: 10, border: "none", fontWeight: 700, cursor: "pointer",
                  background: direction === "owedByMe" ? COLORS.red : COLORS.bgCard2,
                  color: direction === "owedByMe" ? "#fff" : COLORS.textDim,
                }}
              >
                دين عليّ
              </button>
        ...
        {isDual ? (
          <DualCurrencyField label="المبلغ" usd={amount} lbp={amountLBP} onUsdChange={setAmount} onLbpChange={setAmountLBP} />
        ) : (
          <Field label="المبلغ ($)">
            <input style={inputStyle} type="number" inputMode="decimal" value={amount} onChange={(ev) => setAmount(ev.target.value)} placeholder="0.00" autoFocus />
          </Field>
        )}
        <Field label={cfg.reasonLabel}>
          <input style={inputStyle} value={reason} onChange={(ev) => setReason(ev.target.value)} placeholder={cfg.reasonPlaceholder} />
        </Field>
        <Field label="ملاحظات (اختياري)">
          <input style={inputStyle} value={note} onChange={(ev) => setNote(ev.target.value)} placeholder="ملاحظات إضافية" />
        </Field>
        <Field label="التاريخ والوقت">
          <input style={inputStyle} type="datetime-local" value={dateTime} onChange={(ev) => setDateTime(ev.target.value)} />
        </Field>
    ...
    <div>
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        <div style={{ fontSize: 13, color: COLORS.textDim, marginBottom: 16, lineHeight: 1.6 }}>
          تسكير الحساب يحفظ نسخة من الملخص الحالي بتاريخ اليوم في سجل الإقفالات. هذا لا يحذف العمليات ولا يصفّر الأرقام.
        </div>
...
function ConvertScreen({ data, persist, currentUser, onBack, showToast }) {
  ...
  return (
    <div>
      ...
      <div style={{ padding: "0 16px" }}>
        <div style={{ textAlign: "center", color: COLORS.textDim, fontSize: 13, marginBottom: 18 }}>
          سعر الصرف الحالي: 1$ = {fmtLBP(rate)} ل.ل
        </div>

        <div style={{ display: "flex", gap: 8, marginBottom: 18, background: COLORS.bgCard2, borderRadius: 12, padding: 4 }}>
          <button
            onClick={() => setDirection("usd_to_lbp")}
            style={{ flex: 1, padding: "10px", borderRadius: 10, border: "none", fontWeight: 700, cursor: "pointer", background: direction === "usd_to_lbp" ? COLORS.green : "transparent", color: direction === "usd_to_lbp" ? "#fff" : COLORS.textDim }}
          >
            دولار ← ليرة
          </button>
          <button
            onClick={() => setDirection("lbp_to_usd")}
            style={{ flex: 1, padding: "10px", borderRadius: 10, border: "none", fontWeight: 700, cursor: "pointer", background: direction === "lbp_to_usd" ? COLORS.blue : "transparent", color: direction === "lbp_to_usd" ? "#fff" : COLORS.textDim }}
          >
            ليرة ← دولار
          </button>
        </div>
        ...
        {direction === "lbp_to_usd" && amount !== "" && (
          <div style={{ fontSize: 12, color: COLORS.textFaint, marginTop: -8, marginBottom: 14, textAlign: "left" }}>
            = {fmtLBP(amtForCalc)} ل.ل
          </div>
        )}
        ...
        <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20, textAlign: "center", marginTop: 10, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 800, color: direction === "usd_to_lbp" ? COLORS.blue : COLORS.green }}>
            {direction === "usd_to_lbp" ? `${fmtLBP(result)} ل.ل` : `$${fmt(result)}`}
          </div>
        </div>

        <div style={{ fontSize: 12, color: COLORS.textFaint, marginBottom: 12, lineHeight: 1.6 }}>
          تأكيد التحويل يُنقص المبلغ من رصيد {direction === "usd_to_lbp" ? "الدولار" : "الليرة"} ويضيفه لرصيد {direction === "usd_to_lbp" ? "الليرة" : "الدولار"} بالمجموع العام بالرئيسية.
        </div>
  ...
  return (
    <div style={{ padding: "16px 16px 0" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16, background: COLORS.bgCard2, borderRadius: 12, padding: 4 }}>
        {[
          ...
          <button
            ...
            style={{
              flex: 1, padding: "9px 4px", borderRadius: 9, border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer",
              background: period === p.k ? COLORS.yellow : "transparent",
              color: period === p.k ? "#111" : COLORS.textDim,
            }}
          >
            {p.l}
          </button>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, background: COLORS.bgCard, borderRadius: 12, padding: "10px 14px", border: `1px solid ${COLORS.border}` }}>
        <button onClick={() => shift(-1)} style={{ background: "none", border: "none", color: COLORS.text, cursor: "pointer" }}>
          <ChevronRightIcon size={20} />
        </button>
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: "center", color: COLORS.textFaint, padding: "30px 0", fontSize: 14 }}>لا توجد عمليات في هذه الفترة</div>
      )}
    </div>
  );
}
...
function SettingsScreen({ data, persist, currentUser, onBack, onLogout, showToast }) {
  ...
  const saveRate = () => {
    ...
    if (r > 0) {
      persist((prev) => ({ ...prev, exchangeRate: r }));
    }
  ...
  return (
    <div style={{ padding: "16px 16px 0" }}>
      ...
      <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 16, marginBottom: 14, display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 44, height: 44, borderRadius: 99, background: COLORS.green, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 16, marginBottom: 14 }}>
        ...
        {editingRate ? (
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={saveRate} style={{ background: COLORS.green, border: "none", borderRadius: 10, padding: "0 16px", color: "#fff", cursor: "pointer" }}>
              <Check size={18} />
            </button>
          </div>
        ) : (
          <button
            onClick={() => { setRateInput(data.exchangeRate.toString()); setEditingRate(true); }}
            style={{ display: "flex", justifyContent: "space-between", width: "100%", background: COLORS.bgCard2, border: "none", borderRadius: 10, padding: "12px 14px", color: COLORS.text, cursor: "pointer", fontSize: 15 }}
          >
      ...
      <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 16, marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 12, color: COLORS.textFaint, marginTop: 10, lineHeight: 1.6 }}>
          أي مستخدم يسجل دخول من هذا التطبيق يشارك نفس البيانات تلقائياً.
        </div>
      </div>
      ...
      <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 16, marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 12, color: COLORS.textFaint, marginBottom: 12, lineHeight: 1.6 }}>
          يحدّث التيبس والمرتب لكل العمليات القديمة بآخر معادلة وسعر صرف حالي، بدون حذف أو تعديل المبالغ الأصلية.
        </div>
        <button
          onClick={recalcAll}
          style={{ width: "100%", background: COLORS.bgCard2, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: "12px", color: COLORS.text, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontWeight: 700, cursor: "pointer" }}
        >
          <RefreshCw size={16} /> إعادة حساب الآن
        </button>
      </div>
